...
import '../config/app.regex.dart';
import '../enum/client_exception_type.dart';
import '../errors/client_failure.dart';
import '../platform_channel/service/platform_channel_downloader_service.dart';
...
class DirectoryService {
  ...
  Future<int> getApplicationDocumentsDirectorySize() async {
    try {
      ...
      final List<FileSystemEntity> systemEntities =
      ...
      ClientFailure.createAndLog(
        ...
        clientExceptionType:
            ClientExceptionType.deviceDirectoryFetchSizeOperationError,
        exception: ex,
      );
    }
  ...
  Future<int> getTemporaryDirectorySize() async {
    try {
      ...
      final List<FileSystemEntity> systemEntities =
          tempDir.listSync(recursive: true, followLinks: false);
      for (FileSystemEntity entity in systemEntities) {
        if (entity is File) {
          size += await entity.length();
        }
  ...
  Future<void> createApplicationDocumentsDirectoryIfNotExists() async {
    ...
    try {
      ...
      ClientFailure.createAndLog(
        stackTrace: StackTrace.current,
        clientExceptionType:
            ClientExceptionType.deviceDirectoryCreateOperationError,
        exception: ex,
      );
    }
  }
  ...
  Future<bool> deleteTemporaryDirectory() async {
    try {
      ...
      ClientFailure.createAndLog(
        stackTrace: StackTrace.current,
        clientExceptionType:
            ClientExceptionType.deviceDirectoryDeleteOperationError,
        exception: ex,
      );
    }
  ...
  Future<bool> savePublicMediaFile({
    ...
    try {
      ...
      final bool isDownloadSuccessful =
          await _platformChannelDownloaderService.download(
        fileName: fileName,
        data: data,
        mimeType: mimeType,
      );
    ...
    try {
      ...
      if (openFileResult.type == ResultType.noAppToOpen) {
        ClientFailure.createAndLog(
          stackTrace: StackTrace.current,
          clientExceptionType:
              ClientExceptionType.noApplicationFoundToOpenTheFileError,
  ...
  String _addDateTimeInToFileName({required String fileName}) {
    ...
    if (hasExtension) {
      fileNameWithDateTime =
          '${fileName.substring(0, extensionIndex)}_${dateTime.millisecondsSinceEpoch}${fileName.substring(extensionIndex)}';
    } else {
